...
export default function CourseChat({ groupId }: ChatWindowProps) {
  ...
  return (
    <div className="flex flex-col h-[60vh] bg-[#1e1e2e] border border-[#2e2e3e] rounded-xl">
        ...
        {loading ? <p className="text-center text-gray-400">Loading messages...</p> : messages.map(msg => (
          <div key={msg._id} className={`flex my-2 ${msg.senderId === session?.user?.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${
              msg.senderId === session?.user?.id ? 'bg-violet-600 text-white' : 'bg-[#2e2e3e] text-gray-200'
            }`}>
      ...
      <div className="p-4 border-t border-[#2e2e3e]">
        <form onSubmit={handleSendMessage} className="flex items-center gap-2">
          ...
          />
          <button type="submit" className="p-2 bg-violet-600 rounded-full text-white hover:bg-violet-500">
